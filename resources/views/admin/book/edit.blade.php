            ...
            <form action="{{route('books.update',$book)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        ...
                        <br>
                        <img class="img-fluid" src="{{asset('storage/' . $book->cover_image)}}" alt="">
                        @error('cover_image')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="form-group row">
                    <label for="authers" class="col-md-4 col-form-label text-md-right">المؤلفون</label>

                    <div class="col-md-6">
                        <select id="authers" multiple class="form-control" name="authers[]" >
                            <option disabled {{$book->authers()->count() == 0 ?"selected":""}}>اختر المؤلفين</option>
                            @foreach($authers as $auther)
                                <option value="{{ $auther->id }}" {{$book->authers->contains($auther)?"selected" : ""}}>{{ $auther->name }}</option>
                            @endforeach
                        </select>
